...
const UserDetails = ({...props}) => {
  const { toggle } = props;
     ...
     return (
          <section className={`${toggle ? `${style._section_width_resize}` : `${style._section_width}`} mt-[100px] ml-auto bg-[#fbfbfb] relative -z-10 p-6 md:p-[3rem]`}>
               <div>
                    <div className={``}>
               ...
               <div className={`grid md:grid-cols-2 gap-8 lg:grid-cols-4 xl:grid-cols-4 mb-8`}>
                    {UserInfo.map(info => {
                         return (<div className={`bg-[#fff] rounded-[4px] shadow-md border-[1px] border-[rgba(125, 125, 125, 0.06)] p-4  w-full`} key={info.title}>
                         ...
                         <div className={`w-[]`}>
                              <div className={`flex justify-between items-center`}>
                                   {tableHeading.map(heading => {
                                        return <div className={`flex items-center`} key={heading.header}>
